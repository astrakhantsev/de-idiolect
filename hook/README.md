...
**Honest expectations — surfacing is heuristic; the per-term check is the product.** Detection is the entry's weakest measured component: the prototype's own detector missed its one retrospective coinage endpoint (§4.1, §5.3; *"raw keyness under-delivers"*). That detector is **TF-IDF keyness** — a deterministic frequency-contrast statistic (how over-represented a word is in this corpus versus a reference corpus), the right baseline for a *frozen, offline* measurement in which detection was not even wired into the measured path (the evaluated concepts were hand-supplied, so the §5 results do not depend on it). Scan's detector is a **different signal**: a *model* asked which terms an outside expert would not recognize as established vocabulary. That fits a *live* convenience — nondeterminism is fine because the curation gate absorbs it — where a frozen measurement wanted a reproducible statistic instead. Because it is a different, and unmeasured, detector, shipping it **does not** claim to fix the keyness record — it is a convenience that produces a curatable list, gated so its errors cost a curator's glance, not credibility. Expect the raw list to mix real coinages, real-but-off-target coinages, and the occasional junk one; that mix is exactly why the curation gate is not optional. A worked self-application run on this repo's own prose (with `GLOSSARY.md`/`PSEUDOCODE.md`/`hook/` held out as answer-key contamination) is committed under [`example-scan/`](example-scan/), scored by hand against `GLOSSARY.md`'s coinage list.
